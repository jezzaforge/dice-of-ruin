...
const FONT_MAP = {
  cinzel:   { label:"Cinzel (Default)",  css:"'Cinzel', serif",          imp:"Cinzel:wght@400;600;700" },
  uncial:   { label:"Uncial Antiqua",    css:"'Uncial Antiqua', serif",   imp:"Uncial+Antiqua" },
  almendra: { label:"Almendra (Ornate)", css:"'Almendra', serif",         imp:"Almendra:wght@400;700" },
  exo:      { label:"Exo 2 (Sci-fi)",   css:"'Exo 2', sans-serif",       imp:"Exo+2:wght@300;400;700" },
  rajdhani: { label:"Rajdhani (Hive)",  css:"'Rajdhani', sans-serif",    imp:"Rajdhani:wght@400;600;700" },
  roboto:   { label:"Roboto (Clean)",   css:"'Roboto', sans-serif",      imp:"Roboto:wght@300;400;700" },
};
...
const ANTI_KW = ["INFANTRY","VEHICLE","MONSTER","MOUNTED","BEAST","SWARM","FORTIFICATION","CHARACTER","FLY","PSYKER","TITANIC","TYRANID","DAEMON","CHAOS","IMPERIUM","NECRON","ORK","TAU","ELDAR","AELDARI","DRUKHARI","SPACE MARINE"];
  ...
  for(const sel of qAll(force,"selections > selection")) {
    ...
    if(type==="unit") {
      ...
      for(const c of qAll(sel,"selections > selection")) {
        ...
        for(let i=0;i<count;i++) models.push({ instanceId:`${c.getAttribute("id")||mName}-${i}`, name:mName, stats, ranged, melee, alive:true, active:true });
      }
      if(models.length===0 && unitStats) {
        const {ranged,melee}=collectWeapons(sel);
        const count=parseInt(sel.getAttribute("number")||"1");
        for(let i=0;i<count;i++) models.push({ instanceId:`${sel.getAttribute("id")||uName}-${i}`, name:uName, stats:unitStats, ranged, melee, alive:true, active:true });
      }
...
function performRoll({ weapon, numActiveModels, toughness, armorSave, invulnEnabled, invulnSave, halfRange, lanceActive, blastEnemyCount, hitMod, woundMod, targetKeywords, rerollOnesHit, rerollOnesWound, leaderBuffs }) {
  ...
  if (kw.blast) baseAttacks += Math.floor((blastEnemyCount||0) / 5); // +1 per 5 enemy models
  ...
  for (let i=0; i<numAttacks; i++) {
    ...
    // One reroll per hit die — rerollOnes takes priority over leader reroll
    if (!kw.torrent) {
    ...
    for (let h=0; h<hits; h++) {
      ...
      if (autoWound || devWound || antiCrit || normalWound) {
        if (devWound || (antiCrit && !normalWound)) {
          damageDealt.push(rollDmg(weapon.damage)); // no save
        } else {
...
function RollResult({ result, weaponName, color, T, alwaysExpand=false }) {
  ...
  function Arr() { return <div style={{color:T.textFaint,fontSize:14,flexShrink:0,alignSelf:"center",paddingBottom:12}}>›</div>; }
    ...
    <div style={{background:T.panel,border:`1px solid ${color.border}33`,borderLeft:`3px solid ${color.border}`,borderRadius:10,marginBottom:9,boxShadow:`0 2px 14px ${color.glow}`,overflow:"hidden"}}>
      ...
      {open && (
        <div style={{padding:"10px 12px 12px",borderTop:`1px solid ${T.border}`,display:"flex",flexDirection:"column",gap:11}}>
          <DS label={`Hit rolls — ${hitTarget}+`} note={`${hits}/${numAttacks}${rerolledHits.length>0?" · "+rerolledHits.length+" rerolled":""}`} T={T}>
            {hitRolls.map((v,i)=><MiniDie key={i} value={v} success={v>=hitTarget} T={T}/>)}
          </DS>
          {woundRolls.length>0 && <DS label={`Wound rolls — ${woundTarget}+`} note={`${woundsMade}/${woundRolls.length}${rerolledWounds.length>0?" · "+rerolledWounds.length+" rerolled":""}`} T={T}>
          ...
          {damageDealt.length>0 && <DS label="Damage per unsaved wound" note={`${totalDmg} total`} T={T}>
            {damageDealt.map((d,i)=><div key={i} style={{background:"#160606",border:"1px solid #c0392b44",borderRadius:5,padding:"2px 8px",fontFamily:"'Cinzel',serif",fontSize:14,color:"#e74c3c"}}>{d}</div>)}
          </DS>}
        </div>
      )}
    </div>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          {weapon.keywords && weapon.keywords!=="-" && showKeywordBadges && (
            <div style={{display:"flex",flexWrap:"wrap",gap:2,marginTop:2}}>
              {weapon.keywords.split(",").map(k=>k.trim()).filter(Boolean).map((k,idx)=>(
                <span key={idx} style={{fontSize:8,color:T.textDim,background:T.bg,border:`1px solid ${T.border}`,borderRadius:3,padding:"1px 4px"}}>{k}</span>
              ))}
            </div>
          )}
        </div>
        ...
        <div style={{marginLeft:10,marginTop:3,padding:"7px 9px",background:T.bg,border:`1px solid ${T.border}`,borderRadius:6,display:"flex",flexWrap:"wrap",gap:7,alignItems:"center"}}>
          ...
          {kw.blast && (
            <div style={{display:"flex",alignItems:"center",gap:5}}>
              <span style={{fontSize:9,color:T.textDim,fontFamily:"'Cinzel',serif"}}>Blast — enemy models:</span>
          ...
          {kw.twinLinked && <span style={{fontSize:9,color:"#7ec8ff",background:"rgba(126,200,255,0.08)",border:"1px solid rgba(126,200,255,0.2)",borderRadius:3,padding:"2px 6px",fontFamily:"'Cinzel',serif"}}>Twin-Linked: re-rolling wounds</span>}
                ...
                return (
                  <div key={i} style={{background:T.bg,border:`1px solid ${T.border}`,borderRadius:7,padding:"9px 11px",marginBottom:7}}>
                    ...
                    {entry.results.map((r,j)=>{
                      ...
                      return <div key={j} style={{display:"flex",justifyContent:"space-between",padding:"2px 0",borderBottom:`1px solid ${T.border}`}}><span style={{fontSize:10,color:r.color.label,fontFamily:"'Cinzel',serif"}}>{r.weapon.name}</span><span style={{fontSize:10,color:T.text}}>{r.result.numAttacks}A→{r.result.woundRolls.length}H→<span style={{color:"#ff6b6b",fontWeight:700}}>{d}dmg</span></span></div>;
                    })}
                    <div style={{textAlign:"right",marginTop:4}}><span style={{fontFamily:"'Cinzel',serif",fontSize:13,color:"#ff6b6b",fontWeight:900}}>{tot}<span style={{fontSize:9,color:T.textDim,fontWeight:400,marginLeft:3}}>dmg</span></span></div>
                  </div>
                );
              })
          }
    ...
    <div style={{position:"fixed",inset:0,zIndex:100,display:"flex",alignItems:"flex-end",justifyContent:"flex-end",pointerEvents:"none"}}>
      <div style={{width:400,height:"100%",background:T.panel,borderLeft:`1px solid ${T.border}`,boxShadow:"-4px 0 24px rgba(0,0,0,0.6)",pointerEvents:"all",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:13}}>
          {unitKeys.length===0 && allGames.length===0 && <div style={{color:T.textFaint,fontFamily:"'Cinzel',serif",textAlign:"center",marginTop:40,fontSize:11}}>No data yet — roll some dice!</div>}
          {mvpUnit && (
            <div style={{background:`linear-gradient(135deg,${T.accent}22,${T.accent}11)`,border:`1px solid ${T.accent}44`,borderRadius:8,padding:"10px 12px",marginBottom:12}}>
              ...
              <div style={{fontSize:11,color:T.accentText}}>{mvpDmg} total damage</div>
            </div>
          )}
          {unitKeys.length>0 && <>
              ...
              return (
                <div key={k} style={{background:T.bg,border:`1px solid ${T.border}`,borderRadius:7,padding:"9px 11px",marginBottom:7}}>
                  ...
                  <div style={{height:4,background:T.border,borderRadius:2,marginBottom:6,overflow:"hidden"}}>
                    <div style={{height:"100%",width:`${bw}%`,background:T.accent,borderRadius:2}}/>
                  </div>
                  {view==="simple" && (
                    <div style={{display:"flex",gap:12}}>
                      {[["Activations",u.activations],["Attacks",u.totalAttacks],["Hits",u.totalHits],["Wounds",u.totalWounds]].map(([l,v])=>(
                        <div key={l} style={{textAlign:"center"}}><div style={{fontSize:8,color:T.textDim}}>{l}</div><div style={{fontFamily:"'Cinzel',serif",fontSize:13,color:T.text}}>{v||0}</div></div>
                      ))}
                    </div>
                  )}
                  {view==="detailed" && <>
                    <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:6}}>
                      {[["Activations",u.activations],["Attacks",u.totalAttacks],["Hits",u.totalHits],["Wounds",u.totalWounds],["Unsaved",u.totalUnsaved],["Damage",u.totalDamage]].map(([l,v])=>(
                        <div key={l} style={{textAlign:"center",minWidth:44}}><div style={{fontSize:8,color:T.textDim}}>{l}</div><div style={{fontFamily:"'Cinzel',serif",fontSize:12,color:T.text}}>{v||0}</div></div>
                      ))}
                    </div>
          ...
          {allGames.length>0 && <>
            ...
            {[...allGames].reverse().map((g,i)=>{
              ...
              return (
                <div key={i} style={{background:T.bg,border:`1px solid ${T.border}`,borderRadius:7,padding:"9px 11px",marginBottom:7}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                    <span style={{fontFamily:"'Cinzel',serif",color:T.text,fontSize:11,fontWeight:700}}>vs {g.opponent}</span>
            ...
            <button onClick={clearStats} style={{width:"100%",padding:7,marginTop:4,background:"rgba(231,76,60,0.1)",border:"1px solid #e74c3c44",color:"#e74c3c",fontFamily:"'Cinzel',serif",fontSize:10,cursor:"pointer",borderRadius:5}}>Clear All Statistics</button>
          </>}
        </div>
      </div>
    </div>
...
function MenuPanel({ T, theme, setTheme, rosters, setRosters, settings, setSetting, onClose }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:100,background:"rgba(0,0,0,0.65)",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:T.panel,border:`1px solid ${T.border}`,borderRadius:12,padding:20,width:340,maxHeight:"90vh",overflowY:"auto",boxShadow:"0 8px 40px rgba(0,0,0,0.6)"}}>
        ...
        {settings && setSetting && <>
          ...
          <div style={{marginBottom:16}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:5}}>
              {Object.entries(FONT_MAP).map(([key,fm])=>(
                <button key={key} onClick={()=>setSetting("fontFamily",key)} style={{padding:"6px 10px",borderRadius:5,cursor:"pointer",textAlign:"left",border:`1px solid ${settings.fontFamily===key?T.accent:T.border}`,background:settings.fontFamily===key?T.accent+"22":"transparent",color:settings.fontFamily===key?T.accentText:T.textDim,fontFamily:fm.css,fontSize:12}}>{fm.label}</button>
              ))}
            </div>
            <div style={{fontSize:11,color:T.text,margin:"10px 0 6px"}}>Font size</div>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <input type="range" min="12" max="22" step="1" value={settings.fontSize||16} onChange={e=>setSetting("fontSize",parseInt(e.target.value))} style={{flex:1,accentColor:T.accent}}/>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:9,marginBottom:18}}>
            ...
            <SettingToggle label="Show keyword badges" sublabel="Tags like Torrent, Lethal Hits"        value={settings.showKeywordBadges} onChange={v=>setSetting("showKeywordBadges",v)} T={T}/>
          </div>

          <SLabel T={T}>Roll Defaults</SLabel>
          <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:18}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><div style={{fontSize:11,color:T.text}}>Default Toughness</div><div style={{fontSize:9,color:T.textDim}}>Starting value on load</div></div>
              <SmallStepper value={settings.defaultToughness} min={1} max={14} onChange={v=>setSetting("defaultToughness",v)} T={T}/>
            </div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><div style={{fontSize:11,color:T.text}}>Default Armour Save</div><div style={{fontSize:9,color:T.textDim}}>Starting value on load</div></div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><div style={{fontSize:11,color:T.text}}>Default Phase</div></div>
...
function SettingToggle({label,sublabel,value,onChange,T}){
  return (
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12}}>
      <div style={{flex:1}}><div style={{fontSize:11,color:T.text}}>{label}</div>{sublabel&&<div style={{fontSize:9,color:T.textDim,marginTop:1}}>{sublabel}</div>}</div>
...
export default function App() {
  // ── Settings (must be first — everything else may depend on it) ──
  const [settings,setSettings] = useState(()=>loadStorage("dor_settings",DEFAULT_SETTINGS));
  function setSetting(key,val) { setSettings(prev=>{ const next={...prev,[key]:val}; saveStorage("dor_settings",next); return next; }); }

  const [themeKey,setThemeKey] = useState(()=>loadStorage("dor_theme","blood"));
  const T = THEMES[themeKey] || THEMES.blood;
  ...
  useEffect(()=>{
    const fm = FONT_MAP[settings.fontFamily||"cinzel"];
    if(!fm?.imp) return;
    const id = "dor-font-"+(settings.fontFamily||"cinzel");
  ...
  // Pre-compute Anti dropdown (no const inside JSX)
  ...
  const currentFont  = FONT_MAP[settings.fontFamily||"cinzel"]?.css || "'Cinzel', serif";
        ...
        <div className={`main-layout${settings.compactMode?" compact-mode":""}`} style={{display:"flex",flex:1,zIndex:1,overflow:"hidden"}}>
          ...
          <div className="col-controls" style={{width:340,flexShrink:0,overflowY:"auto",padding:13,borderRight:`1px solid ${T.border}`}}>
            {!unit ? (
                ...
                <div>
                  ...
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    {unit.models[0]?.stats && <>
                      ...
                      <Stat label="T"  value={unit.models[0].stats.toughness}    T={T}/>
                      <Stat label="SV" value={unit.models[0].stats.save+"+"}     T={T}/>
                      <Stat label="W"  value={unit.models[0].stats.wounds}       T={T}/>
                      <Stat label="LD" value={unit.models[0].stats.leadership}   T={T}/>
                    </>}
                  </div>
                </div>
                ...
                <Card T={T}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:7}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:7}}>
                      ...
                      {leaderBuffs.sustainedHits && <>
                        <SmallStepper value={leaderBuffs.sustainedHitsVal||1} min={1} max={6} onChange={v=>setLeaderBuffs(lb=>({...lb,sustainedHitsVal:v}))} T={T}/>
                        <span style={{fontSize:8,color:T.textDim}}>hits</span>
                      </>}
                    </div>
                  </div>
                </Card>
              ...
              {/* Roll button */}
              <button onClick={rollAll} disabled={rolling||Object.keys(selectedWeapons).length===0||activeModels.length===0} style={{width:"100%",padding:"14px 0",background:rolling?T.border:`linear-gradient(135deg,${T.accent},${T.accent}dd)`,border:rolling?`1px solid ${T.border}`:`2px solid ${T.accentText}44`,borderRadius:9,color:"#fff",fontFamily:"'Cinzel Decorative','Cinzel',serif",fontSize:14,letterSpacing:4,cursor:rolling?"not-allowed":"pointer",boxShadow:rolling?"none":`0 0 24px ${T.glow},0 4px 12px rgba(0,0,0,0.4)`,textTransform:"uppercase",transition:"all 0.2s",opacity:(Object.keys(selectedWeapons).length===0||activeModels.length===0)?0.4:1}}>
            ...
            {results.length===0
              ...
              : (<>
                  ...
                  <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>
                    ...
                    <button onClick={rerollWounds} style={{flex:1,padding:"6px 8px",background:"transparent",border:`1px solid ${T.border}`,color:T.textDim,fontFamily:"'Cinzel',serif",fontSize:9,cursor:"pointer",borderRadius:5}}>↺ Wounds</button>
                    <button onClick={rerollSaves}  style={{flex:1,padding:"6px 8px",background:"transparent",border:`1px solid ${T.border}`,color:T.textDim,fontFamily:"'Cinzel',serif",fontSize:9,cursor:"pointer",borderRadius:5}}>↺ Saves</button>
                  </div>
